...
# 去除停用词
    ...
    def write_postings_to_db(self):
        ...
        try:
            ...
            c.execute('''CREATE TABLE postings(id int primary key not null auto_increment,
                      term TEXT  not null,
                      df INT,
                      docs TEXT)''')
            c.execute('''alter table postings convert to character set utf8''')
            for key, value in self.postings_lists.items():
                doc_list = '\n'.join(map(str, value[1]))
                sql = "INSERT INTO postings(term,df,docs) VALUES ('%s', '%s', '%s')" % (key, value[0], doc_list)
                c.execute(sql)

            c.execute('''DROP TABLE IF EXISTS doc_len''')
            c.execute('''CREATE TABLE doc_len(id bigint unique not null ,
                                   length double(9,4) not null )''')
            c.execute('''alter table doc_len convert to character set utf8''')
            for doc,cal in self.w_ddic.items():
                sql = "INSERT INTO doc_len VALUES ('%s', '%s')" % (doc, cal)
                c.execute(sql)
